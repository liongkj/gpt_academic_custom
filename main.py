...
help_menu_description = """Github源代码开源和更新[地址🚀](https://github.com/binary-husky/gpt_academic), 
...
def main():
    ...
    from themes.theme import (
        js_code_for_css_changing,
        js_code_for_darkmode_init,
    ...
    title_html = (
        f'<h1 align="center">GPT 学术优化 {get_current_version()}</h1>{theme_declaration}'
    )
    ...
    all_plugin_groups = list(
        set([g for _, plugin in plugins.items() for g in plugin["Group"].split("|")])
    )
    ...
    with gr.Blocks(
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1, elem_id="gpt-panel"):
                with gr.Accordion(
                    "输入区", open=True, elem_id="input-panel"
                ) as area_input_primary:
                    with gr.Row():
                        txt = gr.Textbox(
                            show_label=False,
                    ...
                    with gr.Row():
                        submitBtn = gr.Button(
                            "提交", elem_id="elem_submit", variant="primary"
                        )
                    with gr.Row():
                        resetBtn = gr.Button(
                            "重置", elem_id="elem_reset", variant="secondary"
                        )
                        resetBtn.style(size="sm")
                        stopBtn = gr.Button(
                            "停止", elem_id="elem_stop", variant="secondary"
                        )
                ...
                with gr.Accordion(
                    "基础功能区", open=True, elem_id="basic-panel"
                ) as area_basic_fn:
                    with gr.Row():
                        ...
                        for k in functional:
                            if ("Visible" in functional[k]) and (
                                not functional[k]["Visible"]
                            ):
                                continue
                            variant = (
                                functional[k]["Color"]
                            ...
                            functional[k]["Button"] = gr.Button(
                                k, variant=variant, info_str=f"基础功能区: {k}"
                            )
                            functional[k]["Button"].style(size="sm")
                            predefined_btns.update({k: functional[k]["Button"]})
                with gr.Accordion(
                    "函数插件区", open=True, elem_id="plugin-panel"
                ) as area_crazy_fn:
                    with gr.Row():
                    ...
                    with gr.Row():
                        for k, plugin in plugins.items():
                            ...
                            visible = (
                                True
                                if match_group(plugin["Group"], DEFAULT_FN_GROUPS)
                                else False
                            )
                            variant = (
                                plugins[k]["Color"]
                        ...
                        with gr.Accordion("更多函数插件", open=True):
                            ...
                            with gr.Row():
                                switchy_bt = gr.Button(
                                    r"请先从插件列表中选择", variant="secondary"
                                ).style(size="sm")
                    with gr.Row():
                        with gr.Accordion(
                            "点击展开“文件上传区”。上传本地文件/压缩包供函数插件调用。", open=True
                        ) as area_file_up:
                            file_upload = gr.Files(
                                label="任何文件, 推荐上传压缩文件(zip, tar)",
                                file_count="multiple",
                                elem_id="elem_upload",
                            )
            ...
            with gr.Row():
                ...
                with gr.Tab("更换模型", elem_id="interact-panel"):
                    md_dropdown = gr.Dropdown(
                        AVAIL_LLM_MODELS, value=LLM_MODEL, label="更换LLM模型/请求源"
                    ).style(container=False)
                    top_p = gr.Slider(
                        minimum=-0,
                        maximum=1.0,
                ...
                with gr.Tab("界面外观", elem_id="interact-panel"):
                    theme_dropdown = gr.Dropdown(
                        AVAIL_THEMES, value=THEME, label="更换UI主题"
                    ).style(container=False)
                    ...
                    checkboxes_2 = gr.CheckboxGroup(
                        ["自定义菜单"], value=[], label="显示/隐藏自定义菜单", elem_id="cbsc"
                    ).style(container=False)
                    dark_mode_btn = gr.Button("切换界面明暗 ☀", variant="secondary").style(
                        size="sm"
                    )
                    dark_mode_btn.click(
                        None, None, None, _js=js_code_for_toggle_darkmode
                    )
                with gr.Tab("帮助", elem_id="interact-panel"):
                    gr.Markdown(help_menu_description)

        with gr.Floating(
            init_x="20%", init_y="50%", visible=False, width="40%", drag="top"
        ) as area_input_secondary:
        ...
        with gr.Floating(
            ...
            with gr.Accordion("自定义菜单", open=True, elem_id="edit-panel"):
                with gr.Row() as row:
                    with gr.Column(scale=10):
                        AVAIL_BTN = [btn for btn in customize_btns.keys()] + [
                            k for k in functional
                        ]
                        basic_btn_dropdown = gr.Dropdown(
                            AVAIL_BTN, value="自定义按钮1", label="选择一个需要自定义基础功能区按钮"
                        ).style(container=False)
                        basic_fn_title = gr.Textbox(
                            show_label=False, placeholder="输入新按钮名称", lines=1
                        ).style(container=False)
                        basic_fn_prefix = gr.Textbox(
                            show_label=False, placeholder="输入新提示前缀", lines=4
                        ).style(container=False)
                        basic_fn_suffix = gr.Textbox(
                            show_label=False, placeholder="输入新提示后缀", lines=4
                        ).style(container=False)
                    with gr.Column(scale=1, min_width=70):
                        ...
                        def assign_btn(
                            ...
                            if basic_btn_dropdown_ in customize_btns:
                                ret.update(
                                    {
                                        customize_btns[basic_btn_dropdown_]: gr.update(
                            ...
                            try:
                                persistent_cookie_ = from_cookie_str(
                                    persistent_cookie_
                                )  # persistent cookie to dict
                            except:
                                persistent_cookie_ = {}
                            persistent_cookie_[
                                "custom_bnt"
                            ] = customize_fn_overwrite_  # dict update new value
                            persistent_cookie_ = to_cookie_str(
                                persistent_cookie_
                            )  # persistent cookie to dict
                            ret.update(
                                {persistent_cookie: persistent_cookie_}
                        ...
                        def reflesh_btn(persistent_cookie_, cookies_):
                            ...
                            customize_fn_overwrite_ = persistent_cookie_.get(
                                "custom_bnt", {}
                            )
        ...
        def on_dropdown_changed(k):
            ...
            info = plugins[k].get("Info", k)
            ret = {
                switchy_bt: gr.update(
                    value=k, variant=variant, info_str=f"函数插件区: {info}"
                )
            }
            if plugins[k].get("AdvancedArgs", False):  # 是否唤起高级插件参数区
                ret.update(
                    {
                        plugin_advanced_arg: gr.update(
                            visible=True,
                            label=f"插件[{k}]的高级参数说明："
                            + plugins[k].get("ArgsReminder", [f"没有提供高级参数功能说明"]),
                        )
                    }
                )
            else:
                ret.update(
                    {
                        plugin_advanced_arg: gr.update(
                            visible=False, label=f"插件[{k}]不需要高级参数。"
                        )
                    }
                )
            return ret
        ...
        dropdown.select(
            on_dropdown_changed, [dropdown], [switchy_bt, plugin_advanced_arg]
        )
        ...
        theme_handle = theme_dropdown.select(
            on_theme_dropdown_changed, [theme_dropdown, secret_css], [secret_css]
        )
        theme_handle.then(None, [secret_css], None, _js=js_code_for_css_changing)

        # 随变按钮的回调函数注册
        def route(request: gr.Request, k, *args, **kwargs):
            if k in [r"打开插件列表", r"请先从插件列表中选择"]:
                return
            yield from ArgsGeneralWrapper(plugins[k]["Function"])(
                request, *args, **kwargs
            )
        ...
        def on_group_change(group_list):
            ...
            if not group_list:  # 处理特殊情况：没有选择任何插件组
                return [
                    *[
                        plugin["Button"].update(visible=False)
                        for _, plugin in plugins_as_btn.items()
                    ],
            ...
            for k, plugin in plugins.items():
                if plugin.get("AsButton", True):
                    btn_list.append(
                        plugin["Button"].update(
                            visible=match_group(plugin["Group"], group_list)
                        )
                    )  # 刷新按钮
        ...
        plugin_group_sel.select(
            ...
            outputs=[
                *[plugin["Button"] for name, plugin in plugins_as_btn.items()],
                dropdown,
            ],
        )
        if ENABLE_AUDIO:
        ...
        demo.load(
            None,
            inputs=None,
            outputs=[persistent_cookie],
            _js=js_code_for_persistent_cookie_init,
        )
        demo.load(
            None, inputs=[dark_mode], outputs=None, _js=darkmode_js
        )  # 配置暗色主题或亮色主题
        demo.load(
            None,
    ...
    def run_delayed_tasks():
        ...
        threading.Thread(
            target=auto_updates, name="self-upgrade", daemon=True
        ).start()  # 查看自动更新
        threading.Thread(
            target=open_browser, name="open-browser", daemon=True
        ).start()  # 打开浏览器页面
        threading.Thread(
            target=warm_up_mods, name="warm-up", daemon=True
        ).start()  # 预热tiktoken模块
