...
def 解析源代码新(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    for index, fp in enumerate(file_manifest):
        ...
        i_say_show_user = (
            f'{prefix}[{index}/{len(file_manifest)}] 请对下面的程序文件做一个概述: {fp}'
        )
        # 装载请求内容
    ...
    while True:
        ...
        if last_iteration_result != "":
            sys_prompt_additional = (
                f"已知某些代码的局部作用是:{last_iteration_result}"
        ...
        result = yield from request_gpt_model_in_new_thread_with_ui_alive(
            ...
            sys_prompt=f"你是一个程序架构分析师，正在分析一个项目的源代码。{sys_prompt_additional}",
        )
...
def 解析项目本身(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    history = []    # 清空历史，以免输入溢出
    import glob
    file_manifest = (list(glob.glob('./*.py')) + list(glob.glob('./*/*.py')))
    project_folder = './'
...
def 解析一个Matlab项目(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if os.path.exists(txt):
        ...
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
    file_manifest = list(glob.glob(f'{project_folder}/**/*.m', recursive=True))
    if not file_manifest:
...
def 解析一个前端项目(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    file_manifest = (
        ...
        + list(glob.glob(f'{project_folder}/**/*.vue', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.less', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.sass', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.wxml', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.wxss', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.css', recursive=True))
...
def 解析一个Rust项目(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if os.path.exists(txt):
        ...
        return
    file_manifest = (
        list(glob.glob(f'{project_folder}/**/*.rs', recursive=True))
        + list(glob.glob(f'{project_folder}/**/*.toml', recursive=True))
    ) + list(glob.glob(f'{project_folder}/**/*.lock', recursive=True))
    if len(file_manifest) == 0:
        report_exception(chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何golang文件: {txt}")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
...
def 解析任意code项目(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if maybe_dir and maybe_dir[0].endswith('.extract'):
    ...
    file_manifest = [
        ...
        for f in glob.glob(
            f'{extract_folder_path}/**/{pattern}', recursive=True
        )
        if extract_folder_path != ""
        and os.path.isfile(f)
        and (
            not re.search(pattern_except, f)
            or pattern.endswith(
                '.' + re.search(pattern_except, f).group().split('.')[-1]
            )
        )
    ]
    if not file_manifest:
