...
pj = os.path.join
ARXIV_CACHE_DIR = os.path.expanduser("~/arxiv_cache/")
    ...
    if mode == 'proofread_en':
        ...
        inputs_array = [
            (
                (
                    (
                        f"Below is a section from an English academic paper, translate it into Chinese. {more_requirement}"
...
def desend_to_extracted_folder_if_exist(project_folder):
    ...
    """
    if maybe_dir := [
        f for f in glob.glob(f'{project_folder}/*') if os.path.isdir(f)
    ]:
        return maybe_dir[0] if maybe_dir[0].endswith('.extract') else project_folder
    else:
...
def move_project(project_folder, arxiv_id=None):
    ...
    if not glob.glob(pj(project_folder, '*.tex')) and len(items) == 1:
...
def arxiv_download(chatbot, history, txt, allow_cache=True):
    ...
    if ('.' in txt) and ('/' not in txt) and is_float(txt): # is arxiv ID
        txt = f'https://arxiv.org/abs/{txt.strip()}'
    if ('.' in txt) and ('/' not in txt) and is_float(txt[:10]): # is arxiv ID
        txt = f'https://arxiv.org/abs/{txt[:10]}'
    if not txt.startswith('https://arxiv.org'): 
        return txt, None

    # <-------------- inspect format ------------->
    chatbot.append(["检测到arxiv文档连接", '尝试下载 ...'])
    ...
    dst = pj(translation_dir, f'{arxiv_id}.tar')
...
def Latex英文纠错加PDF对比(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if not os.path.exists(f'{project_folder}/merge_proofread_en.tex'):
...
def Latex翻译中文并重新编译PDF(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if txt.endswith('.pdf'):
        report_exception(chatbot, history, a = f"解析项目: {txt}", b="发现已经存在翻译好的PDF文档")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
    ...
    # <-------------- if merge_translate_zh is already generated, skip gpt req ------------->
    if not os.path.exists(f'{project_folder}/merge_translate_zh.tex'):
        yield from Latex精细分解与转化(file_manifest, project_folder, llm_kwargs, plugin_kwargs, 
