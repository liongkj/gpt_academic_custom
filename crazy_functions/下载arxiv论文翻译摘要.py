...
def download_arxiv_(url_pdf):
    if 'arxiv.org' not in url_pdf:
        if ('.' in url_pdf) and ('/' not in url_pdf):
            new_url = f'https://arxiv.org/abs/{url_pdf}'
    ...
    x = f"{paper_id}  {other_info['year']} {other_info['authors']}.bib"
    x = x.replace('?', '？')\
        .replace(':', '：')\
        .replace('\"', '“')\
        .replace('\n', '')\
...
def get_name(_url_):
    ...
    try:
        ...
        for item in comment.replace('\n', ' ').split('   '):
            if 'Comments' in item:
                real_comment = item
        other_details['comment'] = real_comment if real_comment is not None else ''
    except:
    ...
    return f'{title_str}.pdf', other_details
...
def 下载arxiv论文并翻译摘要(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    try:
        ...
        report_exception(
            ...
            history,
            a=f"解析项目: {txt}",
            b="导入软件依赖失败。使用该模块需要额外依赖，安装方法```pip install --upgrade beautifulsoup4```。",
        )
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
    ...
    try:
        ...
        report_exception(chatbot, history, a = f"解析项目: {txt}", b="下载pdf文件未成功")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
